...
module.exports = {
     ...
     *
     * @param addressData a json of the following structure (street1, city, state are required):
         {
            "name": "Shawn Ippotle",
            "company": "Shippo",
     ...
     * @param apiToken
     * @param callback, with data of the following structure:
         {
            "address": {
               "name": "Shawn Ippotle",
               "company": "Shippo",
    ...
    getShippingRate: function (rateRequestData, apiToken, callback) {
        ...
        let shipmentRequestData = {
            address_from: requestData.addressFrom,
            address_to: requestData.addressTo,
            parcels: [requestData.parcel],
            async: false
        };
        if (requestData.addressReturn) {
            shipmentRequestData.addressReturn = requestData.addressReturn;
        }
        if (requestData.extra) {
            shipmentRequestData.extra = requestData.extra;
        }

        shippo.shipment.create(shipmentRequestData)
        .then(data => {
            callback(null, data);
        })
        .catch(err => {
