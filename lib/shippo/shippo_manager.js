...
function handleShippoError(err) {
    if (err.type === 'ShippoAuthenticationError') {
        ...
        return new CustomError('500-01-001', err.message);
    }
}
...
module.exports = {
    ...
    validateAddress: function (addressData, apiToken, callback) {
        ...
        shippo.address.create(requestData)
            .then(data => {
                logger.debug('Shippo returned data successfully');
            ...
            .catch(err => {
                logger.error('Error encountered while requesting from Shippo: ' + err);
     ...
     * @param shipmentData An object of the following structure (parcel, addressTo, and addressFrom are required):
         {
            ...
            "options": {
                "carrierAccounts": ["0d3181c72e884756b13bcc17a11146f0"],
     ...
     * @param callback
     */
    getShippingRate: function (shipmentData, apiToken, callback) {
        const shippo = getShippo(apiToken || FLEXE_API_TOKEN);
        let rateRequestData = {
        ...
        .catch(err => {
            logger.debug('Error encountered while requesting from Shippo: ' + err);
            let customError = handleShippoError(err);
            callback(customError);
        });
    },
