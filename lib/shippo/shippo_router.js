...
module.exports = {
    doAddressValidation: function (req, res) {
        ...
        if (!isAddressProperlyFormatted(address)) {
            createResponse.failure(req, res, new CustomError('400-02-003', { name: 'address' }));
            return;
        }
        const shippoApiToken = common.getIn(req, 'body.data.shippoApiToken');
        shippoManager.validateAddress(address, shippoApiToken, null, (err, data) => {
            if (req.timedout) {
                return;
            }
            if (err) {
    ...
    getShippingRate: function (req, res) {
        ...
        if (!isAddressProperlyFormatted(shipmentData.addressFrom)) {
            createResponse.failure(req, res, new CustomError('400-02-003', { name: 'addressFrom' }));
            return;
        }
        if (!isAddressProperlyFormatted(shipmentData.addressTo)) {
            createResponse.failure(req, res, new CustomError('400-02-003', { name: 'addressTo' }));
            return;
        }
        if (shipmentData.addressReturn && !isAddressProperlyFormatted(shipmentData.addressReturn)) {
            createResponse.failure(req, res, new CustomError('400-02-003', { name: 'addressReturn' }));
            return;
        }
        ...
        const shippoApiToken = common.getIn(req, 'body.data.shippoApiToken');
        shippoManager.getShippingRate(shipmentData, shippoApiToken, null, (err, data) => {
            if (req.timedout) {
                return;
            }
            if (err) {
